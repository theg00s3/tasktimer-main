...
export function getPomodorosForDay (day) {
  return (dispatch, getState) => {
    ...
    .then(data => {
      console.log('data', data)
      if (data.error) {
        getState().user && AnalyticsService.track('get-pomodoro-failure', data.error)
        return dispatch({type: GET_POMODOROS_FOR_DATE_ERROR, payload: data.error})
      }
      getState().user && AnalyticsService.track('get-pomodoro-success', data)
      dispatch({type: GET_POMODOROS_FOR_DATE_SUCCESS, payload: data})
    })
    .catch(err => {
      getState().user && AnalyticsService.track('get-pomodoro-failure', err)
